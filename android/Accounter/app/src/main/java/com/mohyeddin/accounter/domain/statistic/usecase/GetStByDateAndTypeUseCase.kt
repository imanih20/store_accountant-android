...
class GetStByDateAndTypeUseCase(private val rep: StatisticRepository) {
    suspend operator fun invoke(type: String,year:Int,month: Int) : Flow<BaseResult<List<Statistic>, WrappedListResponse<StatisticResponse>>>{
        return rep.getStByDateAndType(type, year, month)
    }
}